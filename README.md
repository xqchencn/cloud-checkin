...
- 站点 JSON 导入和导出；带账号标签的数据按 URL + 账号标签匹配，历史数据缺少账号标签时才按站点名称和 URL 匹配。
...
```text
[scheduled-dev] local scheduled simulator ready: 0 */2 * * * | 0 17 * * *
[scheduled-dev] next local scheduled trigger: "0 */2 * * *" at 2026-05-03 04:00 UTC
```

到点触发时会继续输出：

```text
[scheduled-dev] local scheduled trigger due: "0 */2 * * *" at 2026-05-03 04:00 UTC
[scheduled-dev] scheduled trigger sent for "0 */2 * * *"
```
